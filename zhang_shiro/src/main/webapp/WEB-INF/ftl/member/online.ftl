...
<head>
    ...
    <script>
        ...
        function changeSessionStatus(sessionIds, status, self) {
            ...
            //loading
            var load = layer.load();
            $.post("${basePath}/member/changeSessionStatus.shtml", {
                status: status,
        ...
        <div class="col-md-10">
            ...
            <form method="post" action="" id="formId" class="form-inline">
                <div clss="well">
                    ...
                    <p>
                        再者，说明一个问题，老有同学纠结怎么删除无效的Session，也就是删除用户直接关闭浏览器，导致无法继续使用的Session，我再次严正声明，这个Session不需要你删除，别纠结了，这个Session是有TTL，有效期是30分钟，30分钟这个Session没有更新就会剔除，故你不用纠结。</p>
                            ...
                            <td>
                                ...
                                <@shiro.hasPermission name="/member/changeSessionStatus.shtml">
                                    <a v="onlineDetails" href="javascript:void(0);" sessionId="${it.sessionId}"
                                       status="${(it.sessionStatus)?string(1,0)}">${(it.sessionStatus)?string('踢出','激活')}</a>
                                </@shiro.hasPermission>
                            </td>
                        </tr>
                    </#list>
